        ...
        private static async Task PeoplePickerExample(IActiveDirectoryClient client, string searchString)
        {
            ...
            if (usersList != null && usersList.Count > 0)
            {
                do
                {
                    ...
                    {
                        //var thumbnailPhoto = await user.ThumbnailPhoto.DownloadAsync();
                        Console.WriteLine("User {0} DisplayName: {1}  UPN: {2} Photo: {3} Mail: {4}",
                            index, user.DisplayName, user.UserPrincipalName, string.IsNullOrEmpty(user.ThumbnailPhoto.ContentType) ? "no": "yes", user.Mail);
                        index++;
                    }
            ...
            IPagedCollection<IGroup> groupResults = null;
            List<IGroup> groupsList = null;
            try
            {
                IGroupCollection groupCollection = client.Groups;
                groupResults = await groupCollection.Where(w => w.DisplayName == "Boost_test2").Take(10).ExecuteAsync();
                groupsList = groupResults.CurrentPage.ToList();
            }
            catch (Exception e)
            {
                Program.WriteError("\nError getting group {0}", Program.ExtractErrorMessage(e));
            }
            if (groupsList != null && groupsList.Count > 0)
            {
                do
                {
                    int index = 1;
                    groupsList = groupResults.CurrentPage.ToList();
                    foreach (IGroup group in groupsList)
                    {
                        var groupFetcher = (IGroupFetcher)group;
                        var members = groupFetcher.Members.ExecuteAsync().Result.CurrentPage.ToList();
                        Console.WriteLine("Group {0} DisplayName: {1} Members: {2}",
                            index, group.DisplayName, string.Join(";", members.Select(m => { var user = m as IUser; var th = user.ThumbnailPhoto; return user.UserPrincipalName + " " + user.DisplayName + th.ContentType; } )));
                        index++;
                    }
                    groupResults = await groupResults.GetNextPageAsync();
                } while (groupResults != null);
            }
            else
            {
